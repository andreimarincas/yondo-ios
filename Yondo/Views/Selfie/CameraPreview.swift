...
struct CameraPreview: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> PreviewView {
        Log.debug("🎥 CameraPreview makeUIView - creating PreviewView")
        let preview = PreviewView()
        Log.debug("🔌 Connecting preview source to PreviewView")
    ...
    func updateUIView(_ previewView: PreviewView, context: Context) {
        Log.debug("🔄 CameraPreview updateUIView called")
    }
    ...
    class PreviewView: UIView, PreviewTarget {
        ...
        nonisolated func setSession(_ session: AVCaptureSession) {
            ...
            Log.debug("🎬 setSession called - session: \(ObjectIdentifier(session))")
            Task { @MainActor in
                Log.debug("🎬 Binding AVCaptureSession to previewLayer")
                previewLayer.session = session
                
                CATransaction.begin()
                CATransaction.setDisableActions(true)
                previewLayer.videoGravity = .resizeAspectFill
                Log.debug("📐 Preview layer configured (resizeAspectFill)")
...
struct DefaultPreviewSource: PreviewSource {
    ...
    func connect(to target: PreviewTarget) {
        Log.debug("🔗 DefaultPreviewSource connecting session to target")
